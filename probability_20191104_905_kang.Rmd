...
If we want to know the probability of two events, say $A$ and $B$, occuring, we can use the multiplication rule:

<center>$\text{Pr} (A\ \text{and}\ B) = \text{Pr} (A) \text{Pr} (B\ |\ A)$</center>
...
<center>$\text{Pr} (A\ \text{and}\ B\ \text{and}\ C) = \text{Pr} (A) \text{Pr} (B\ |\ A) \text{Pr} (C\ | A\ \text{and}\ B)$</center>
...
<center>$\text{Pr} (A\ \text{and}\ B\ \text{and}\ C) = \text{Pr} (A) \text{Pr} (B) \text{Pr} (C)$</center>
...
<center>$\text{Pr} (B\ |\ A) = \cfrac{\text{Pr} (A\ \text{and}\ B)}{\text{Pr} (A)}$</center>
...
<center>$\text{Pr} (A\ \text{or}\ B) = \text{Pr} (A) + \text{Pr} (B) - \text{Pr} (A\ \text{and}\ B)$</center>
...
[Venn diagram](https://rafalab.github.io/dsbook/book_files/figure-html/venn-diagram-addition-rule-1.png)
...
<center>$\cfrac{\text{Pr} (A\ \text{and}\ B)}{\text{Pr} (A)}$</center>
...
(hands[1] %in% aces & hands[2] %in% facecard) |
   (hands[2] %in% aces & hands[2] %in% facecard)
```
...
<center>$1 \times \cfrac{364}{365} \times \cfrac{363}{365} \cdot\cdot\cdot \cfrac{365 - n + 1}{365}$</center>
...
B <- 10000
draw_without <- replicate(B, sample(balls, 15))
first <- draw_without[1,]
second <- draw_without[2,]
mean(first %in% "cyan" & second %in% c("magenta", "yellow")) / mean(first %in% "cyan")
```
...
```{r}
draw_with <- replicate(B, sample(balls, 2, replace = TRUE))
first_with <- draw_with[1,]
second_with <- draw_with[2,]
mean(first_with %in% "cyan" & second_with %in% c("magenta", "yellow")) / mean(first_with %in% "cyan")
```

5. Two events $A$ and $B$ are independent if $\text{Pr} (A\ \text{and}\ B) = \text{Pr} (A) \text{Pr} (B). Under which situation are the draws independent?
...
B <- 10000
championship_win <- replicate(B, b())
mean(championship_win >= 4)
```
