...
select(wide_data, country, `1960`:`1965`)
```
...
**1**.Examine ths built-in dataset `co2`. Which of the following is true:
```{r}
head(co2)
```
```{r}
...
```
  ...
  **A. `co2` is tidy data: it has one year for each row.**
  ...
  B. `co2` is not tidy: we need at least on column with a character vector.
...
<center>original data -> select -> filter</center>
...
```{r}
mutate(murders, rate = total / population * 10^5, rank = rank(-rate)) %>% select(state, rate,rank)
```
