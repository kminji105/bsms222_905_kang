...
We want to know the probability that the interval $[\bar{X} - 2\hat{\text{SE}}(\bar{X}),\ \bar{X} - 2\hat{\text{SE}}(\bar{X})]$ contains the true proportion $p$. First, consider that the start and end of these intervals are random variables: every time we take a sample, they change. To illustrate this, run the Monte Carlo simulation above twice. We use the same parameters as above:
...
<center>$\text{Pr} \left (\bar{X} - 1.96\hat{\text{SE}}(\bar{X}) \le p \le \bar{X} + 1.96\hat{\text{SE}}(\bar{X}) \right )$</center>

By subtracting and dividing the same quantities in all parts of the equation, we get that the above is equivalent to:

<center>$\text{Pr} \left (-1.96 \le \dfrac{\bar{X} - p}{\hat{\text{SE}}(\bar{X})} \le 1.96 \right )$</center>
...
<center>$\text{Pr}(-1.96 \le Z \le 1.96)$</center>
...
<center>$\text{Pr}(-z \le Z \le z) = 0.99$</center>
...
```{r}
p <- 0.482
poll <- poll %>% mutate(hit = lower <= p & upper >= p)
```
...
```{r}
pnorm(qnorm(1-(1-p)/2)) - pnorm(-qnorm(1-(1-p)/2))
```
...
p <- 0.482
d <- -(2*p - 1)
polls <- polls %>% mutate(hit = d_hat >= -d & d_hat <= d)
```
...
```{r}
poll_difference <- abs(polls$d_hat - 0.021)
qplot(polls$d_hat, poll_difference)
```

10. Redo the plot that you made for exercise 9, but only for pollsters that took five or more polls.

```{r}
pollsters <- polls %>% filter(enddate - startdate >= 5)
poll_difference <- abs(pollsters$d_hat - 0.021)
qplot(pollsters$d_hat, poll_difference)
```
...
includes 0. If this were a poll and we were forced to make a declaration, we would have to say it was a “toss-up”.ㄴ
...
<center>$\text{Pr}(|\ \bar{X} - 0.5\ | > 0.02)$</center>

assuming the $2p - 1 = 0$ or $p = 0.5$. Under the null hypothesis we know that:

<center>$\sqrt{N} \dfrac{\bar{X} - 0.5}{\sqrt{0.5(1 - 0.5)}}$</center>

is standard normal. We therefore can compute the probability above, which is the p-value.

<center>$\text{Pr} \left ( \sqrt{N} \dfrac{|\bar{X} - 0.5|}{\sqrt{0.5(1 - 0.5)}} > \sqrt{N} \dfrac{0.02}{\sqrt{0.5(1 - 0.5)}} \right )$</center>
...
<center>$\text{Pr}(Y = 1\ |\ X = 1)/\text{Pr}(Y = 0\ |\ X = 1)$</center>
...
<center>$\text{Pr}(Y = 1\ |\ X = 0)/\text{Pr}(Y = 0\ |\ X = 0)$</center>
...
<center>$\sqrt{1/a + 1/b + 1/c + 1/d}$</center>

This implies that a 95% confidence interval for the log odds ratio can be formed by:

<center>$log \left ( \dfrac{ad}{bc} \right ) \pm 1.96 \sqrt{1/a + 1/b + 1/c + 1/d}$</center>
...
```{r}
reason <- data.frame(winning = c("yes", "no"),
                     career = (500 * c(0.7, 0.3)),
                     Olympics = (17 * c(8/17, 9/17)))
chisq_test <- reason %>% select(-winning) %>% chisq.test()
chisq_test
```
...
```{r}
odds_career <- with(reason, (career[2]/sum(career)) / (career[1]/sum(career)))
odds_Olympics <- with(reason, (Olympics[2]/sum(Olympics)) / (Olympics[1]/sum(Olympics)))
log_or <- log(odds_Olympics / odds_career)
se <- reason %>% select(-winning) %>%
  summarize(se = sqrt(sum(1/Olympics) + sum(1/career))) %>%
  pull(se)
losing_under_pressure <- log_or + c(-1,1) * qnorm(0.975) * se
losing_under_pressure
```
...
```{r}
m_reason <- reason %>% select(-winning) %>%
  mutate(career = career * 2, Olympics = Olympics * 2)
m_reason %>% chisq.test() %>% .$p.value
```

```{r}
odds_career <- with(m_reason, (career[2]/sum(career)) / (career[1]/sum(career)))
odds_Olympics <- with(m_reason, (Olympics[2]/sum(Olympics)) / (Olympics[1]/sum(Olympics)))
log_or <- log(odds_Olympics / odds_career)
se <- m_reason %>%
  summarize(se = sqrt(sum(1/Olympics) + sum(1/career))) %>%
  pull(se)
