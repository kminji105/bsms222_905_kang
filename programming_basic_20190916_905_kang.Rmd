...
```{r}
base::nchar('District of Columbia')
```
...
```{r}
new_names <- ifelse(base::nchar(murders$state) > 8, murders$abb, murders$state)
```
...
```{r}
c <- 1:25
s_n <- purrr::map_dbl(c, compute_s_n)
plot(c, s_n)
```
