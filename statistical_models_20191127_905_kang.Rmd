...
Since the 2008 elections, other organizations have started their own election forecasting group that, like Nate Silver’s, aggregates polling data and uses statistical models to make predictions. In 2016, forecasters underestimated Trump’s chances of winning greatly. The day before the election the _New York Times_ reported[$^{55}$](https://rafalab.github.io/dsbook/models.html#fn55) the following probabilities for Hillary Clinton winning the presidency:

| - | NYT | 538 | HuffPost | PW | PEC | DK | Cook | Roth |
| - | - | - | - | - | - | - | - | - |
| Win Prob | 85% | 71% | 98% | 89% | >99% | 92% | Lean Dem | Lean Dem |

For example, the Princeton Election Consortium (PEC) gave Trump less than 1% chance of winning, while the Huffington Post gave him a 2% chance. In contrast, FiveThirtyEight had Trump’s probability of winning at 29%, higher than tossing two coins and getting two heads. In fact, four days before the election FiveThirtyEight published an article titled _Trump Is Just A Normal Polling Error Behind Clinton_[$^{56}$](https://rafalab.github.io/dsbook/models.html#fn56). By understanding statistical models and how these forecasters use them, we will start to understand how this happened.
...
$s = \sqrt{\textstyle \sum_{i=1}^{N} {(X_i - \bar{X})}^2 / (N - 1)}$.
...
```{r}
N <- 50
male_height <- sample(x, size = N, replace = TRUE)
mu <- mean(male_height)
sigma <- sd(male_height)
mu
sigma
```
...
```{r}
sigma <- sqrt(sum({male_height - mean(male_height)}^2 / (N - 1)))
sigma
```
...
```{r}
s <- sqrt(sum({male_height - mean(male_height)}^2 / (N - 1)))
se <- s / sqrt(N)
...
N <- 50
B <- 10000
male_h_avg <- replicate(B, {
  male_height <- sample(x, size = N, replace = TRUE)
  mu <- mean(male_height)
  s <- sd(male_height)
  se <- s / sqrt(N)
  between(mu, mu - 1.96 * se, mu + 1.96 * se)
})
mean(male_h_avg)
```
...
polls %>% ggplot(aes(spread)) +
  geom_histogram(color = "black", binwidth = .009)
```
...
<center>$Y_{i,j} = d + b_i + \varepsilon_{i,j}$</center>
...
9. In the right side of the model only $\varepsilon_{i,j}$ is a random variable. The other two are constants. What is the expected value of $Y_{1,j}$?
...
polls %>%
  ...
  summarize(expected_value = mean(spread))
```
...
```{r}
polls %>% filter(pollster == "The Times-Picayune/Lucid") %>%
  summarize(N_2 = n())
```
...
```{r}
polls %>% group_by(pollster) %>%
  summarize(sigma = sqrt(sum({spread - mean(spread)}^2 / (n() - 1))))
```
...
difference_ci <- polls %>%
  group_by(pollster) %>%
  summarize(avg = mean(spread),
            sigma = sqrt(sum({spread - mean(spread)}^2 / (n() - 1))),
            se = sigma / sqrt(n())) %>%
  ungroup() %>%
  summarize(d_avg = avg[2] - avg[1],
            d_sigma = sigma[2] - sigma[1],
            d_se = se[2] - se[1]) %>%
  mutate(start = d_avg - 1.96 * d_se,
         end = d_avg + 1.96 * d_se)
round(difference_ci * 100, 1)
```
...
p_value <- polls %>%
  ...
  summarize(avg = mean(spread),
            sigma = sqrt(sum({spread - mean(spread)}^2 / (n() - 1))),
            se = sigma / sqrt(n())) %>%
  select(-pollster, -sigma)
round(p_value * 100, 1) %>% chisq.test() %>% .$p.value
```

20. The statistic formed by dividing our estimate of $b_2 - b_1$ by its estimated standard error:

<center>$\dfrac{\bar{Y}_2 - \bar{Y}_1}{\sqrt{s_2^2 / N_2 + s_1^2 / N_1}}$</center>
...
polls %>%
  ...
  summarize(avg = mean(spread),
            sigma = sqrt(sum({spread - mean(spread)}^2 / (n() - 1))),
            se = sigma / sqrt(n())) %>%
  summarize(variability = mean(avg), s = sd(avg))
```
