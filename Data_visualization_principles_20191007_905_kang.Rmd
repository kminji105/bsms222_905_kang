...
<center>![](https://rafalab.github.io/dsbook/dataviz/img/class2_8.jpg)</center>
...
Here is another example, described in detail in this [Folowing Data post](http://flowingdata.com/2012/08/06/fox-news-continues-charting-excellence/)

<center>![](https://rafalab.github.io/dsbook/dataviz/img/Bush-cuts.png)</center>  
...
<center>![](https://rafalab.github.io/dsbook/dataviz/img/venezuela-election.png)</center>
...
<center>![](https://rafalab.github.io/dsbook/dataviz/img/class2_30.jpg)</center>  
...
> coord는 `coordinate`의 약자, filp은 뒤집다의 뜻을 가짐
> 따라서 `coord_filp()`은 x축과 y축의 구성을 뒤집어 표현하라는 명령어이며, 가로 막대로 값을 표현하는 bar 그래프가 그려짐
...
> alpha blending은 points를 투명하게 만드는 것 (alpha = )
...
Notice how much more we learn from the two plots on the right.

> Barplots are useful for showing one number, but not very useful when we want to describe distributions.
...
![color_blind_fridendly](https://rafalab.github.io/dsbook/book_files/figure-html/color-blind-friendly-colors-1.png)
...
One exception where another type of plot may be more informative is when you are

> comparing variables of the same type, but at different time points and for a relatively small number of comparisons.

For example, comparing life expectancy between 2010 and 2015. In this case, we might recommend a _slope chart_.
...
<center>![](https://rafalab.github.io/dsbook/dataviz/img/fig8b.png)</center>
...
<center>![](https://rafalab.github.io/dsbook/dataviz/img/fig1e.png)</center>
...
Note that happens when we make a barplot:
...
us_contagious_diseases %>%
  ...
  mutate(rate = count / population * 10000 * 52 / weeks_reporting) %>%
  ggplot(aes(year, rate)) +
  geom_line(aes(group = state), color = "grey50", alpha = 0.2) +
  geom_line(mapping = aes(year, us_rate), data = us, color = "black") +
  scale_y_sqrt()
```
...
sp %>% ggplot(aes(year, state, fill = rate)) +
  geom_tile(color = "grey50") +
  scale_x_continuous(expand = c(0, 0)) +
  scale_fill_gradientn(colors = RColorBrewer::brewer.pal(9, "Blues"), trans = "sqrt") +
  geom_vline(xintercept = 1938, col = "red") +
  theme_minimal() +
  theme(panel.grid = element_blank()) +
  ggtitle(the_disease) +
  xlab("") +
...
> Smallpox는 Measles에 비해 vaccine이 일찍 보급되었다고 볼 수 있다
...
> `theme_minimal`은 no background annotations
> 잘 보면 plot background color가 다르다
...
```{r}
us_contagious_diseases %>%
  filter(!state %in% c("Hawaii", "Alaska") & state == "California" & weeks_reporting > 10) %>%
  mutate(rate = count / population * 10000 * 52 / weeks_reporting) %>%
  mutate(state = reorder(state, rate)) %>%
  filter(!is.na(rate)) %>%
...
us_contagious_diseases %>%
  ...
  ggplot(aes(year, rate)) +
  geom_line(aes(group = state), color = "grey50", alpha = 0.2) +
  geom_line(mapping = aes(year, us_rate), data = us2, color = "black", size = 1) +
  geom_vline(xintercept = 1963, col = "Red") +
  scale_y_sqrt() +
  xlab("") +
  ylab("") +
  ggtitle("Cases per 10,000 by state") +
  geom_text(data = data.frame(x = 1957, y = 55),
            mapping = aes(x, y, label = "US average"),
            color = "black")
```
