...
> rnorm : 정규분포 내 값을 자동으로 원하는 갯수만큼 구해준다  
> pnorm : 정규분포 내 값에 따른 확률을 구해준다  
> qnorm : 정규분포 확률에 해당하는 값을 알려준다  
...
![probability distribution](https://rafalab.github.io/dsbook/book_files/figure-html/plot-of-height-frequencies-1.png)
...
<center>$\text{Pr} (X = 4) = 1/6$</center>

The CDF can then easily be defined:

<center>$F(4) = \text{Pr} (X \le 4) = \text{Pr} (X = 4) + \text{Pr} (X = 3) + \text{Pr} (X = 2) + \text{Pr} (X = 1)$</center>

Although for continuous distributions the probability of a single value $\text{Pr} (X = x)$ is not defined, there is a theoretical definition that has a similar interpretation. The probability density at $x$ is defined as the function $f(a)$ such that:

<center>$F(a) = \text{Pr} (X \le a) = \int_{-\infty}^{a} f(x) dx$</center>

For those that know calculus, remember that the integral is related to a sum: it is the sum of bars with widths approximating 0. If you don’t know calculus, you can think of $f(x)$ as a curve for which the area under that curve up to the value $a$, gives you the probability $\text{Pr} (X \le a)$.
...
![probability height](https://rafalab.github.io/dsbook/book_files/figure-html/intergrals-1.png)
...
Nor surprisingly, the distribution looks normal:

![distribution](https://rafalab.github.io/dsbook/book_files/figure-html/simulated-heights-1.png)
...
![distribution](https://rafalab.github.io/dsbook/book_files/figure-html/simulated-tallest-height-1.png)
...
mean = 64 * 2.54
sd = 3 * 2.54
pnorm(67*2.54, mean, sd) - pnorm(61*2.54, mean, sd)
```
...
<center>$\text{Pr} (X \le a)$</center>

  Now we subtract $\mu$ to both sides and then divide both sides by $\sigma$:

<center>$\text{Pr} \left ( \cfrac{X - m}{s} \le \cfrac{a - m}{s} \right )$</center>
...
<center>$\text{Pr} \left ( Z \le \cfrac{a - m}{s} \right )$</center>
