...
1. The expected value of the sum of random variables is the sum of each random variable’s expected value. We can write it like this:
   ...
   <center>$\text{E}[X_1 + X_2 + \cdots + X_n] = \text{E}[X_1] + \text{E}[X_2] + \cdots + \text{E}[X_n]$</center>

   If the $X$ are independent draws from the urn, then they all have the same expected value. Let’s call it $\mu$ and thus:

   <center>$\text{E}[X_1 + X_2 + \cdots + X_n] = n\mu$</center>
...
2. The expected value of a non-random constant times a random variable is the non-random constant times the expected value of a random variable. This is easier to explain with symbols:
   ...
   <center>$\text{E}[aX] = a \times \text{E}[X]$</center>
   ...
   <center>$\text{E}[(X_1 + X_2 + \cdots + X_n)/n] = \text{E}[X_1 + X_2 + \cdots + X_n]/n = n\mu/n = \mu$</center>
...
3. The square of the standard error of the sum of **independent** random variables is the sum of the square of the standard error of each random variable. This one is easier to understand in math form:
   ...
   <center>$\text{SE}[X_1 + X_2 + \cdots + X_n] = \sqrt{\text{SE}[X_1]^2 + \text{SE}[X_2]^2 + \cdots + \text{SE}[X_n]^2}$</center>
...
4. The standard error of a non-random constant times a random variable is the non-random constant times the random variable’s standard error. As with the expectation:
   ...
   <center>$\text{SE}[aX] = a \times \text{SE}[X]$</center>
...
<center>$\text{SE}[(X_1 + X_2 + \cdots + X_n)/n]\\ = \text{SE}[X_1 + X_2 + \cdots + X_n]/n \\= \sqrt{\text{SE}[X_1]^2 + \text{SE}[X_2]^2 + \cdots + \text{SE}[X_n]^2}/n\\ = \sqrt{{\sigma}^2 + {\sigma}^2 + \cdots + {\sigma}^2}/n\\ = \sqrt{n{\sigma}^2}/n = \sigma/\sqrt{n}$</center>
...
B <- 1000
winnings <- function(x){
   S <- sample(c(17,-1), x, replace = TRUE, prob = c(1/19, 18/19))
   sum(S)
}
outcomes_6 <- replicate(B, winnings(6))
outcomes_7 <- replicate(B, winnings(7))
...
If $\text{Pr}(S < 0) = 0.01$ then

<center>$\text{Pr} \left( \dfrac{S - \text{E}[S]}{\text{SE}[S]} < \dfrac{-\text{E}[S]}{\text{SE}[S]} \right)$</center>

And remember $\text{E}[S]$ and $\text{SE}[S]$ are the expected value and standard error of $S$, respectively. All we did above was add and divide by the same quantity on both sides. We did this because now the term on the left is a standard normal random variable, which we will rename $Z$. Now we fill in the blanks with the actual formula for expected value and standard error:

<center>$\text{Pr} \left( Z < \dfrac{-\{lp + x(1 - p)\}n}{(x - l)\sqrt{np(1 - p)}} \right) = 0.01$</center>
...
<center>$x = -l \dfrac{np - z\sqrt{np(1 - p)}}{n(1 - p) + z\sqrt{np(1 - p)}}$</center>
...
<center>$\text{Pr}(S < 0) = \text{Pr} \left(Z < -\dfrac{\text{E}[S]}{\text{SE}[S]} \right)$</center>

with $Z$ a standard normal random variable as shown earlier. If we define $\mu$ and $\sigma$ to be the expected value and standard deviation of the urn, respectively (that is of a single loan), using the formulas above we have: $\text{E}[S] = n\mu\ \text{and} \ \text{SE}[S] = \sqrt{n}\sigma$. So if we define $z =$ `qnorm(0.01)`, we have:

<center>$-\dfrac{n\mu}{\sqrt{n}\sigma} = -\dfrac{\sqrt{n}\mu}{\sigma} = z$</center>

which implies that if we let:

<center>$n \ge z^2{\sigma}^2 / {\mu}^2$</center>
...
<center>$\text{SE}[(X_1 + X_2 + \cdots + X_n)/n] = \sigma/\sqrt{n}$</center>
...
<center>$\text{SE}[(X_1 + X_1 + \cdots + X_1)/n] = \text{SE}[nX_1/n] = \sigma > \sigma/\sqrt{n}$</center>
...
loss_per_foreclosure <- -200000
p <- 0.3
S <- sample(c(0,1), 10000, prob = c(1-p, p), replace = TRUE)
```
...
5. Suppose we give out loans for \$180,000. What should the interest rate be so that our expected value is 0?
    ...
    > interest rate = 0일 때 expected value = 0이 된다.

```{r}
p <- 0.5
S <- sample(c(0,1), 180000, prob = c(1-p, p), replace = TRUE)
mean(S)
```
...
6. (Harder) What should the interest rate be so that the chance of losing money is 1 in 20? In math notation, what should the interest rate be so that $\text{Pr}(S < 0) = 0.05$?
...
l <- loss_per_foreclosure
z <- qnorm(0.05)
x <- -l * (10000 * p - z * sqrt(10000 * p * (1-p))) / (10000 * (1-p) + z * sqrt(10000 * p * (1-p)))
x
```
