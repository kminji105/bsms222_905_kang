...
We are almost there! All we have left to do is add color, a lengend and optional changes to the style.
...
> geom_text(aes())를 이용하여 label을 넣을 수 있음(geom_label()도 가능, ggplot에 넣는것도 가능)
...
> `geom_label`과 `geom_text`로 했을때의 결과가 다름  
> `geom_point()`를 사용하지 않았기 때문에 point는 나타나지 않은 것  
> aes를 이용하여 ggplot & geom_label 어느쪽에 지정해도 결과는 같다
...
```{r}
murders %>% ggplot(aes(population, total)) +
  geom_text(aes(label = abb), color = "blue")
```

> 색 지정을 해줄때 대문자, 소문자는 상관없다
...
> ggplot이나 geom_label 아무것이나 aes만 써서 그룹별 색을 지정해주면 상관없다
